import { blobToText, throwException, ProductDto, API_BASE_URL, SessionDto } from './service-proxies';
...
export class PlateMenuServiceProxy{
    ...
    generatePlateMenu(input: ReplicateInput): Observable<boolean> {
        let url_ = this.baseUrl + "/api/services/app/PlateMenus/GeneratePlateMenu";
        url_ = url_.replace(/[?&]$/, "");
        ...
        return this.http.request("post", url_, options_).flatMap((response_: any) => {
            return this.processGeneratePlateMenu(response_);
        }).catch((response_: any) => {
            if (response_ instanceof HttpResponseBase) {
                try {
                    return this.processGeneratePlateMenu(<any>response_);
                } catch (e) {
                    return <Observable<boolean>><any>Observable.throw(e);
                }
            } else
    ...
    protected processGeneratePlateMenu(response: HttpResponseBase): Observable<boolean> {
        const status = response.status;
        const responseBlob =
            response instanceof HttpResponse ? response.body :
                (<any>response).error instanceof Blob ? (<any>response).error : undefined;
