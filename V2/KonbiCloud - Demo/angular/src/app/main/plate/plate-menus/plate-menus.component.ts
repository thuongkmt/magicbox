...
import { PlateMenuServiceProxy, PlateMenuDto, PlateMenuInputDto, ImportResult, ImportData, ReplicateInput } from '@shared/service-proxies/plate-menu-service-proxies';
...
export class PlateMenusComponent extends AppComponentBase {
            ...
            this.papa.parse(fileContent, {
                ...
                complete: (result) => {
                    ...
                    for (let record of result.data) {
                        ...
                        item.categoryCode = record["Category Code"];
                        item.plateCode = record["Plate Code"];
    ...
    replicateData() {
        this.replicateModal.show(this.dateFilter, this.sessionId)
    }
    ...
    generateData() {
        ...
        input.dateFilter = this.dateFilter;
        this._plateMenusServiceProxy.generatePlateMenu(
            input
        ).finally(() => {
            this.primengTableHelper.hideLoadingIndicator();
        })
