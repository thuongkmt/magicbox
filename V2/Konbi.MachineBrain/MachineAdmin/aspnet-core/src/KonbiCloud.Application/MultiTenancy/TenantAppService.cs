...
namespace KonbiCloud.MultiTenancy
{
    ...
    public class TenantAppService : KonbiCloudAppServiceBase, ITenantAppService
    {
        ...
        [AbpAllowAnonymous]
        public async Task<int> GetTenantIdByHostName(string hostName)
        {
            try
            {
                hostName = hostName.Trim('w').Trim('.');
                var tenant = hostName.Substring(0, hostName.IndexOf(".")).ToLower();
                if (!string.IsNullOrEmpty(tenant))
                {
                    var data = await TenantManager.Tenants.Where(x => x.Name.ToLower().Equals(tenant)).FirstOrDefaultAsync();
                    return data?.Id??0;
                }
                return 0;
            }
