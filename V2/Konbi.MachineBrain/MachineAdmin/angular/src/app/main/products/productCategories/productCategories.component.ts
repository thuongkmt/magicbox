...
export class ProductCategoriesComponent extends AppComponentBase {
    ...
    syncFromCloud(): void{
        this.message.confirm(
            ...
            (isConfirmed) => {
                if (isConfirmed) {
                    this._productCategoriesServiceProxy.sendSyncCategoriesRequest()
                        .subscribe(() => {
                            this.reloadPage();
        ...
        this._productCategoriesServiceProxy.getAll(
            ...
            9999999
                ...
                for (let record of result.items) {
                    csvData.push({
                        Name: (record.productCategory.name == null) ? '' : record.productCategory.name,
                        Code: (record.productCategory.code == null) ? '' : record.productCategory.code,
                        Price: (record.productCategory.desc == null) ? '' : record.productCategory.desc
                    });
                }
                // Export csv data.
