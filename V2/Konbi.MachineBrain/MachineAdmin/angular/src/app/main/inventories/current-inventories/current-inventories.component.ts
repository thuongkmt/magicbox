...
@Component({
  selector: 'app-current-inventories',
  templateUrl: './current-inventories.component.html',
  styleUrls: ['./current-inventories.component.css'],
  animations: [appModuleAnimation()]
})

export class CurrentInventoriesComponent extends AppComponentBase implements OnInit, OnDestroy {
  ...
  public inventoryItemDto: GetInventoryForViewDto[] = [];
  ...
  getAllItems(event?: LazyLoadEvent) {
    ...
    this.topicSubscription = this.rxStompService.watch('/topic/inventory').subscribe((message: Message) => {
      ...
      this.inventories.forEach((inven) => {
          ...
          inventory.tagId = inven.TagId;
          inventory.price = inven.Price;
          inventory.id = inven.Id;
  ...
  UpdateCloudInventories() {
    this.message.confirm(
      '',
      (isConfirmed) => {
        if (isConfirmed) {
          this._httpClient.post(this._baseUrl + '/api/services/app/Inventories/SyncInventoriesToCloud', null).subscribe(data => {
             this.notify.success(this.l('Successfully Deleted'));
           }, error => console.error(error));
        }
      }
