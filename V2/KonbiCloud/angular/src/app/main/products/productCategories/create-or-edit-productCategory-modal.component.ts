...
import { CategoryServiceProxy, CreateOrEditProductCategoryDto } from '@shared/service-proxies/category-service-proxies';
...
export class CreateOrEditProductCategoryModalComponent extends AppComponentBase {
    ...
    show(productCategoryId?: string): void {
        if (!productCategoryId) {
            ...
            this._categoriesServiceProxy.getProductCategoryForEdit(productCategoryId).subscribe(result => {
    ...
    save(): void {
            this.saving = true;
            this._categoriesServiceProxy.createOrEdit(this.productCategory)
