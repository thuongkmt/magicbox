...
using KonbiCloud.Core.Dependency;
using KonbiCloud.Data;
using KonbiCloud.Localization.Resources;
using KonbiCloud.Services.Navigation;
...
namespace KonbiCloud
{
    ...
    {
        static CloudDatabase database;
        public static CloudDatabase Database
        {
            get
            {
                if (database == null)
                {
                    database = new CloudDatabase(Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData), "CloudUrl.db3"));
                }
                return database;
            }
        }
