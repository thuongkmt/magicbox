...
namespace KonbiCloud.Migrations.Seed.Tenants
{
    public class TenantRoleAndUserBuilder
    {
        ...
        public TenantRoleAndUserBuilder(KonbiCloudDbContext context, int tenantId)
        {
            _context = context;
            _tenantId = tenantId;
        }

        public TenantRoleAndUserBuilder(KonbiCloudDbContext context)
        {
            _context = context;
        }
        ...
        public void CreateRestockerRole()
        {
            var tenantIds = _context.Tenants.Select(x => x.Id).ToList();

            foreach(var id in tenantIds)
            {
                var restockerRole = _context.Roles.IgnoreQueryFilters().FirstOrDefault(r => r.TenantId == id && r.Name == StaticRoleNames.Tenants.Restocker);
                if (restockerRole == null)
                {
                    _context.Roles.Add(new Role(id, StaticRoleNames.Tenants.Restocker, StaticRoleNames.Tenants.Restocker) { IsStatic = true, IsDefault = true });
                    _context.SaveChanges();
                }
            }
        }
