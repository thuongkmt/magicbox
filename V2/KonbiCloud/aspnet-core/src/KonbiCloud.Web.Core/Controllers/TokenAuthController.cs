...
using KonbiCloud.Authentication.TwoFactor.Google;
using KonbiCloud.Authorization;
using KonbiCloud.Authorization.Users;
using KonbiCloud.MultiTenancy;
using KonbiCloud.Web.Authentication.JwtBearer;
using KonbiCloud.Web.Authentication.TwoFactor;
using KonbiCloud.Web.Models.TokenAuth;
using KonbiCloud.Authorization.Impersonation;
...
namespace KonbiCloud.Web.Controllers
{
    ...
    public class TokenAuthController : KonbiCloudControllerBase
    {
        ...
        [HttpGet]
        public async Task<RestockerTokenDto> GetTokenByPassCode(string passCode,int tenantId)
        {
            using (_unitOfWorkManager.Current.DisableFilter(AbpDataFilters.MayHaveTenant, AbpDataFilters.MustHaveTenant))
            {
                var user = await _userRepository.FirstOrDefaultAsync(x => x.PassCode == passCode && x.TenantId == tenantId);

                if (user == null) return null;

                //var isRestocker = await UserManager.IsInRoleAsync(user, "Restocker");
                //if (!isRestocker) return null;

                //var impersonationToken = await _impersonationManager.GetImpersonationToken(user.Id, tenantId);

                //var result = await _impersonationManager.GetImpersonatedUserAndIdentity(impersonationToken);
                //var accessToken = CreateAccessToken(await CreateJwtClaims(result.Identity, result.User));

                return new RestockerTokenDto
                {
