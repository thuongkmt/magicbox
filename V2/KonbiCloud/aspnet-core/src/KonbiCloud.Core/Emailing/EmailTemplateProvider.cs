...
namespace KonbiCloud.Emailing
{
    public class EmailTemplateProvider : IEmailTemplateProvider, ISingletonDependency
    {
        ...
        public string GetDefaultTemplate(int? tenantId)
        {
            ...
            lock (SyncObj)
            {
                using (var stream = typeof(EmailTemplateProvider).GetAssembly().GetManifestResourceStream("KonbiCloud.Emailing.EmailTemplates.default.html"))
                {
                    var bytes = stream.GetAllBytes();
                    var template = Encoding.UTF8.GetString(bytes, 3, bytes.Length - 3);
                    template = template.Replace("{THIS_YEAR}", Clock.Now.Year.ToString());
