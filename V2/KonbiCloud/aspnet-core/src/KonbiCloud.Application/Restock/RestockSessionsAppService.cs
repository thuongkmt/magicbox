...
namespace KonbiCloud.Restock
{
    ...
    public class RestockSessionsAppService : KonbiCloudAppServiceBase, IRestockSessionsAppService
    {
        ...
        public RestockSessionsAppService(IRepository<RestockSession> restockSessionRepository,
            IRestockSessionsExcelExporter restockSessionsExcelExporter,
        ...
        public async Task<PagedResultDto<GetRestockSessionForViewDto>> GetAll(GetAllRestockSessionsInput input)
        {
            ...
            var filteredRestockSessions = _restockSessionRepository.GetAll()
                        ...
                        .WhereIf(input.MinRestockedFilter != null, e => e.Restocked >= input.MinRestockedFilter)
                        .WhereIf(input.MaxRestockedFilter != null, e => e.Restocked <= input.MaxRestockedFilter)
                        .WhereIf(input.MinUnloadedFilter != null, e => e.Unloaded >= input.MinUnloadedFilter)
                        .WhereIf(input.MaxUnloadedFilter != null, e => e.Unloaded <= input.MaxUnloadedFilter);
        ...
        [AbpAllowAnonymous]
        public async Task<int> CreateAndGetId(CreateOrEditRestockSessionDto input, Guid machineId, int tenantId)//, Guid machineId, int? tenantId)
        {
            try
            {
                var restockSession = ObjectMapper.Map<RestockSession>(input);
                restockSession.TenantId = tenantId;
                restockSession.MachineId = machineId;

                var id = await _restockSessionRepository.InsertAndGetIdAsync(restockSession);
                return id;
            }
            catch (Exception ex)
            {
                _detailLogService.Log(ex.ToString());
                return 0;
            }
        }
        ...
        protected virtual async Task Create(CreateOrEditRestockSessionDto input)
        {
            ...
            {
                restockSession.TenantId = (int?)AbpSession.TenantId;
            }
